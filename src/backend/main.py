...
try:
    ...
    if not os.path.exists(DB_PATH):
        logger.error(f"File does not exist at DB_PATH: {DB_PATH}")
        logger.info(f"Listing directory contents at {DATA_DIR}: {os.listdir(DATA_DIR)}")
        raise FileNotFoundError(f"File not found: {DB_PATH}")
    else:
        logger.info("DB_PATH exists")
    ...
    logger.info("Module import for main.py completed successfully")

except Exception as e:
    logger.error(f"Failed to load main.py module: {str(e)}")
    raise